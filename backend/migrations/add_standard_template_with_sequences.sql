-- Add Standard Seven-Chapter Template (With Proper Sequence Management)
-- First fixes all sequences, then adds the template

DO $$
DECLARE
    sector_id INTEGER := 1; -- Use Digital Therapeutics sector
    template_id INTEGER;
    chapter_id INTEGER;
    max_template_chapter_id INTEGER;
    max_chapter_question_id INTEGER;
BEGIN
    -- Fix all sequences first to prevent conflicts
    RAISE NOTICE 'Fixing database sequences...';
    
    -- Fix healthcare_sectors sequence
    PERFORM setval('healthcare_sectors_id_seq', (SELECT COALESCE(MAX(id), 1) FROM healthcare_sectors));
    
    -- Fix analysis_templates sequence 
    PERFORM setval('analysis_templates_id_seq', (SELECT COALESCE(MAX(id), 1) FROM analysis_templates));
    
    -- Fix template_chapters sequence
    SELECT COALESCE(MAX(id), 1) INTO max_template_chapter_id FROM template_chapters;
    PERFORM setval('template_chapters_id_seq', max_template_chapter_id);
    
    -- Fix chapter_questions sequence
    SELECT COALESCE(MAX(id), 1) INTO max_chapter_question_id FROM chapter_questions;
    PERFORM setval('chapter_questions_id_seq', max_chapter_question_id);
    
    RAISE NOTICE 'Sequences fixed. template_chapters: %, chapter_questions: %', max_template_chapter_id, max_chapter_question_id;
    
    -- Check if template already exists
    SELECT id INTO template_id FROM analysis_templates 
    WHERE name = 'Standard Seven-Chapter Review';
    
    -- Only insert if template doesn't exist
    IF template_id IS NULL THEN        
        -- Insert the standard template (sequence should now work properly)
        INSERT INTO analysis_templates (
            ...
            TRUE,
            FALSE,  -- Don't make it default to avoid conflicts
            0
        ...
        RAISE NOTICE 'Successfully created standard template with ID % and all chapters and questions!', template_id;
        
    ELSE
        RAISE NOTICE 'Template "Standard Seven-Chapter Review" already exists with ID: %', template_id;
    END IF;
