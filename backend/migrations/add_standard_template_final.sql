-- Add Standard Seven-Chapter Template (Final Version)
-- Uses existing healthcare sector and handles ID sequence properly
...
BEGIN
    ...
    IF template_id IS NULL THEN
        -- Get next available ID by finding max ID + 1
        SELECT COALESCE(MAX(id), 0) + 1 INTO template_id FROM analysis_templates;
        
        -- Insert the standard template with explicit ID
        ...
        PERFORM setval('analysis_templates_id_seq', template_id);
