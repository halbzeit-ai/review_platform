...
SELECT DISTINCT
    pd.company_id,
    COALESCE(u.company_name || ' - Initial Round', 'Initial Funding Round') as project_name,
    'initial' as funding_round,
    
    -- Extract funding_sought from ai_analysis_results JSON if available
    CASE 
        WHEN pd.ai_analysis_results IS NOT NULL THEN
            pd.ai_analysis_results->>'funding_sought'
        ELSE NULL
    ...
    CASE 
        WHEN pd.ai_analysis_results IS NOT NULL THEN
            pd.ai_analysis_results->>'company_offering'
        ELSE NULL
