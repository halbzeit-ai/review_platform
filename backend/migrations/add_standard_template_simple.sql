-- Add Standard Seven-Chapter Template (Simple Version)
-- This template reflects the default review structure used in the pitch deck analyzer
-- Uses the first existing healthcare sector
...
DECLARE
    ...
    chapter_id INTEGER;
BEGIN
    -- Get the first available healthcare sector
    SELECT id INTO sector_id FROM healthcare_sectors WHERE is_active = TRUE ORDER BY id LIMIT 1;
    
    IF sector_id IS NULL THEN
        RAISE EXCEPTION 'No active healthcare sectors found. Please create a sector first.';
    END IF;
    
    RAISE NOTICE 'Using healthcare sector ID: %', sector_id;
    
    -- Check if template already exists
    SELECT id INTO template_id FROM analysis_templates 
    WHERE name = 'Standard Seven-Chapter Review' AND is_active = TRUE;
    ...
    IF template_id IS NULL THEN
        ...
        RAISE NOTICE 'Successfully created standard template with all chapters and questions!';
